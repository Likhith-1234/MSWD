...
const App = () => {
  ...
  const addFormData = () => {
    ...
    personServices
      ...
      .then(
        setNotification({
          message: `Succesfully Added ${newData.name}.`,
        })
      )
      .catch((error) => console.warn(error));
  };
  ...
  const updateFornData = () => {
    const confirmNumberReplace = window.confirm(
      `${newName} is already in phonebook, Do you want to replace with old one.`
    );
    ...
    if (confirmNumberReplace) {
      personServices
        ...
        .then((response) => {
          ...
          setNotification({
            message: `Succesfully replaced ${newName}'s number.`,
          });
        })
        .catch((error) => {
          console.warn(error);
    ...
    if (confirmDelete) {
      personServices
        ...
        .then(() => {
          ...
          setNotification({
            message: `Succesfully Deleted ${name}'s contact`,
          });
        })
        .catch((error) => {
          setNotification({
